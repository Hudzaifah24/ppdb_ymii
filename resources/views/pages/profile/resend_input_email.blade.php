...
<html lang="en">
  <head>
    ...
    <title>YMII | Kirim Ulang Kode</title>
          ...
          <div class="container z-10">
            <div class="flex flex-wrap justify-center -mx-3">
              <div class="w-full max-w-full px-3 mx-auto mt-0 text-center lg:flex-0 shrink-0 lg:w-5/12">
                ...
                <p class="text-white">Masukkan Email dan Username terdaftar anda.</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="w-full max-w-full px-3 mx-auto mt-0 md:flex-0 shrink-0 md:w-7/12 lg:w-5/12 xl:w-4/12">
              <div class="relative z-0 flex flex-col min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
                <div class="flex-auto p-6">
                    <form role="form text-left" method="POST" action="{{ route('resend.action') }}">
                        @csrf
                        @method('POST')
                    ...
                    @endif
                    <div class="mb-4">
                        <input name="username" type="text" class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="Masukkan Username Anda" aria-label="username" aria-describedby="username-addon" />
                        {{-- @if (Session::has('errorUsername'))
                            <p class="text-red-500 text-sm">{{ Session::get('errorUsername') }}</p>
                        @endif --}}
                    </div>
                    <div class="mb-4">
                        <input name="email" type="email" class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="Masukkan Email Anda" aria-label="email" aria-describedby="email-addon" />
                    </div>

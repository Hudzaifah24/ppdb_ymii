...
<html lang="en">
  <head>
    ...
    <title>YMII | Ijazah Terakhir</title>
        ...
        <div class="flex items-center justify-between w-full px-6 py-1 mx-auto flex-wrap-inherit">
          <nav>
            ...
            <ol class="flex flex-wrap pt-1 pl-2 pr-4 mr-12 bg-transparent rounded-lg sm:mr-16">
              ...
              <li class="text-sm pl-2 capitalize leading-normal before:float-left before:pr-2 before:content-['/']" aria-current="page">Upload Ijazah Terakhir</li>
            </ol>
            <h6 class="mb-2 ml-2 font-bold text-white capitalize">Upload Ijazah Terakhir</h6>
          </nav>
            ...
            <div class="w-full max-w-full px-3 lg-max:mt-6 xl:w-full">
                <div class="relative flex flex-col h-full min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
                    @if ($user->Application->ijazah_terakhir != NULL)
                        <a href="{{ asset('assets/img/documents/'.$user->Application->ijazah_terakhir) }}" target="__blank">
                            <img class="object-cover h-full w-full rounded-lg" src="{{ $user->Application->ijazah_terakhir ? asset('assets/img/documents/'. $user->Application->ijazah_terakhir) : '' }}">
                        </a>
                    @endif
                  <div class="p-4 pb-0 mb-0 bg-white border-b-0 rounded-t-2xl">
                    <div class="flex flex-wrap -mx-3">
                      <div class="flex items-center w-full max-w-full px-3 shrink-0 md:w-8/12 md:flex-none">
                        <h6 class="mb-0">Ijazah Terakhir</h6>
                      </div>
                    </div>
                  </div>
                  <div class="flex-auto p-4">
                    <ul class="flex flex-col pl-0 mb-0 rounded-lg">
                        <form role="form text-left" action="{{ route('it.proses', $user->username) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('POST')
